...
;
; PURPOSE:
;   This code computes a VSK interpolant based on a first image reconstruction
;
; CALLING SEQUENCE:
;   int_vsk, vis, usampl, N, Ulimit, ep, aux_data, imsize
;
; INPUTS:
...
;          Oct 2024, Volpara A. adapted to RIS approach

function int_vsk, vis, trigger_map, usampl, N, Ulimit, ep, aux_data, imsize
  ...
  if imsize[0] LT 130L then begin
    trigger_map = make_map(trigger_map.data,$
      dx=1.,dy=1.)
    newmap = rep_tag_value(trigger_map, fltarr(imsize),'DATA')
    newmap.dx = 1. & newmap.dy = 1.
    trigger_map = inter_map(trigger_map, newmap, err=err)
  endif else begin
    trigger_map = make_map(trigger_map.data,$
      dx=1.,dy=1.)
    newmap = rep_tag_value(trigger_map, fltarr(imsize),'DATA')
  ...
  if (vis[0].type EQ 'stx_visibility') then begin
    map_dummy = rot_map(make_map(trigger_map.data),aux_data.ROLL_ANGLE,rcenter=[0.,0.])
    map_coarse =   rotate(map_dummy.data,1)
    px_1 = trigger_map.dx
    dx = px_1[0]
    dy =  dx
  endif else begin
    map_coarse =  rotate(trigger_map.data,1)
    px_1 = trigger_map.dx
    dx = px_1[0]
    dy =  px_1[0]
  endelse
