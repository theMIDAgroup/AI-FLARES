...
;     1) check the sampling (it must be uniform), check the high and low energy values
...
;   electron_bin:                - Width (keV) of the electron energy bins (= width of the count energy bins)
...
pro vis_reg_ge_rescaling,photon_energy_low_array,photon_energy_high_array,$
    vis_array,vis_uncertainty_array,drmini,electron_bin,$
    el_energy_max_factor,nph,eps,gs,err,wei,fitph,fitel,drm
...
;;;;;;;; Check the sampling (it must be uniform)
photon_bin_position=0.5
unif=1
bin=photon_energy_low_array[1]-photon_energy_low_array[0]
for i=2,N-1 do begin
    if (photon_energy_low_array[i]-photon_energy_low_array[i-1] ne bin) then unif=0  ;; sampling non uniform
end
if (unif eq 1) then epsini=photon_energy_low_array+bin*photon_bin_position else message, 'Sampling must be uniform'
...
;;;;; select in the drm matrix the submatrix corresponding to energy between
;;;;; eps(Nstart)-eps(Nend)
...
nee=ceil((eemax-eps0[0])/electron_bin+1)
ee=findgen(nee)*electron_bin+eps0[0]
visfitel=dblarr(nee)
...
eebrem=0
Z=1.2
vis_reg_ge_cross3bn,eebrem,nph,nee,eps0,ee,drm0,visfit0,visfitel,Z,w,u,sf
...
while (rerr gt total(arg0)) do begin
    ...
    nee=ceil((eemax-eps1[0])/electron_bin+1)
    ee=findgen(nee)*electron_bin+eps1[0]
    visfitel1=dblarr(nee)
    ...
    eebrem=0
    vis_reg_ge_cross3bn,eebrem,nph,nee,eps1,ee,drm1,visfit1,visfitel1,Z,w,u,sf
