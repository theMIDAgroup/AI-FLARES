...
;
;  GOPT
;     - Count visibility spectrum corresponding to the recovered electron visibility array
;
; REGSOL
;     - The recovered electron visibility array
;
; RESIDUAL_ARRAY
;     - Residual count visibility spectrum (actual - recovered)
;
...
for l=0,iterations-1 do begin
    ...
    if (opt le optmin) then opt=lambda ;;;;; if after 100 iterations the 3-sigma criterion is not
...
for k=0,nph-1 do begin
    scal=(wei*gs)##u[k,*]
    for j=0,nee-1 do arg1[k,j]=scal*w[k]*sf[k,j]/(w[k]*w[k]+opt)
end
