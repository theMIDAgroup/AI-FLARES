...
class access_db:  # Read only(erase the write), reads the records from the database
    ...
    def load_dataset(self, offline=False):
        if not offline:
            ...
            self.write_property_files()
        else:
            self.read_property_files()
    ...
    def load_dataset_creazione_pkl(self, offline=False):

        self.read_property_files_creazione_pkl()
        # Build the label dataframe and complete the feature dataframe ()
        self.build_dataset()
        # Clean point in time (l.1 level 1, i.e. clean data)
        self.load()
    ...
    def write_property_files(self):
        if not os.path.exists('./data'):
            os.makedirs('./data')

        if self.phase == 'training':
            self.feature_dirty_df.to_pickle('data/features_2012_2015.pkl')
            ...
            json.dump(
                ...
                open(
                    'data/flare_association_2012_2015' + # str(self.flare_class) +
                    '.json',
                    'w'))
        # 'data/flare_association_2012_2016' +  # str(self.flare_class) +

        elif self.phase == 'execution':
            self.feature_dirty_df.to_pickle('data/features_2016_2017.pkl')
            # self.feature_dirty_df.to_pickle('data/features_2017_09_event.pkl')
            # numpy.save('data/features_2015.npy', self.feature_dirty_df)
            json.dump(
                self.flare_association,
                open(
                    'data/flare_association_2016_2017' + # str(self.flare_class) +
                    '.json',
                    'w'))

    # 'data/flare_association_2017_09_event' +  # str(self.flare_class) +

    def read_property_files_creazione_pkl(self):

        self.feature_dirty_df = pandas.read_pickle(
                'data/features_with_topo_2012_2014.pkl')
        df2 = pandas.read_pickle(
            'data/features_with_topo_2015_2016.pkl')
        self.feature_dirty_df = self.feature_dirty_df.append(df2)
            # self.feature_dirty_df = numpy.load('data/features_2013_2014.npy').item()
        self.flare_association = json.load(open(
                'data/flare_association_2012_2014.json','r'))
        b = json.load(open(
                'data/flare_association_2015_2016.json','r'))
        self.flare_association.extend(b)

    def read_property_files(self):
        if self.phase == 'training':
            self.feature_dirty_df = pandas.read_pickle(
                'data/features_2012_2014.pkl')
            # self.feature_dirty_df = numpy.load('data/features_2013_2014.npy').item()
            self.flare_association = json.load(open(
                'data/flare_association_2012_2014.json','r'))
        elif self.phase == 'execution':
            self.feature_dirty_df = pandas.read_pickle(
                'data/features_2015_2016.pkl')
            ...
            for entry in tmp_data:
                ...
                for key, item in self.vector_features.items():
                    if key in entry_flat['data']:
                        entry_flat['data'][key] = entry_flat['data'][key][str(item)]
