...
def classification_skills(y_real, y_pred):
    ...
    return cm.tolist(), far, pod, acc, hss, tss, fnfp, hss1


def metrics_classification(y_real, y_pred, print_skills=False):

    cm, far, pod, acc, hss, tss, fnfp, hss1 = classification_skills(y_real, y_pred)
...
class PKMeans_HM:
    ...
    def fit(self, X_training, Y_training):
        ...
        labels_for_classification = check_prediction_clustering(Y_training, self.estimator.labels_)
        self.metrics_training = metrics_classification(Y_training, labels_for_classification)
...
class RandomForest:
    ...
    def predict(self, X_testing, Y_testing=None):
        ...
        if self.classification == 'hss' or self.classification == 'tss' or self.classification == 'hybrid':
            ...
            Y_testing_prediction = self.estimator.predict_proba(X_testing)
